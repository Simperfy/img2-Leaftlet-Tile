...
async function img2LeafletTile({
  ...
  if (zoomLevels.length <= 0)
    throw new Error("Please provide zoomLevels array");
  ...
  for (const zoomLevel of zoomLevels) {
    ...
    if (resize) logger(`Resize to: ${resize}`);
    ...
    for (let a = 0; a < xLimit / cropDimensionWidth; a++) {
      ...
      for (let b = 0; b < yLimit / cropDimensionHeight; b++) {
        ...
        if (resize) {
          ...
          img = sharp(buffer).resize(resizeWidth, resizeHeight);
        }
